...
首先将两个集合用 `bitset` 表示某个元素是否在集合中，然后按照某个元素的出现次数（$ \mathrm{count} $）分块，块大小为 $ \mathrm{size} $。
对于 $ \mathrm{count} < \mathrm{size} $，暴力贡献所有的集合对，插入：$ O(\mathrm{size}^2 \log \mathrm{size}) $，查询：$ O(q \log \mathrm{size}) $；
对于 $ \mathrm{count} \ge \mathrm{size} $，在每次查询时遍历这些元素，$ O(q \frac{N}{\mathrm{size} \cdot \omega}) $。
最小化总复杂度，$ \mathrm{size} \approx e^{W(N q \omega)} $（在题目范围下 $ size \approx 2000 $，直接当根号就行）
...
对于所有询问，直接暴力哈希匹配，只不过对于 $ n < \mathrm{size} $，对查询结果记忆化，这是考虑到字符串数量较少时，每个串的长度相对较长；数量较多时，每个串长度相对较短，且__由于是前缀/后缀匹配，两个串比较的执行次数取决于较短的串__。

对于 $ n < \mathrm{size} $，$ O(\mathrm{size}^2 \frac{S}{\mathrm{size}}) = O(\mathrm{size} \cdot S) $；
对于 $ n \ge \mathrm{size} $，$ O(q \frac{S}{\mathrm{size}}) $。
最小化总复杂度，$ \mathrm{size} \approx \sqrt{q} $，总复杂度：$ O(S \sqrt{q}) $
...
而在一般图中，考虑对度数分块，则每个点只需要考虑其对大点（$ \frac{n}{\mathrm{size}} $ 个）的贡献，当然，小点在统计时要遍历 $ \mathrm{size} $ 个邻居。[5214]
...
则对于两个 $ \mathrm{DFN} $ 临近的同颜色点，在它们的 $ \mathrm{LCA} $ 打 $ -1 $ 标记。
...
# KMP
$ \mathrm{next} $ 数组记录模式串  时最大的 $ j(0<=j<i,\mathrm{无匹配:}-1) $，自我匹配时前缀和后缀偏移至少为 $ 1 $（否则直接自我开头匹配到结尾，无意义），所以应从 $ i = 1 $ 而不是 $ i = 0 $ 开始自我匹配。
匹配成功后继续移动 $ \mathrm{fail} $ 指针避免漏掉该位置的其它匹配。
...
选择一个恰当的 $ \mathrm{null} $ 值，这个值需要能够与其它“存在”状态区分开，不一定是 $ 0 $，因为有时候空状态也必须要记录下来（如记忆化搜索）。
...
在按 $ \mathrm{DFN} $ 处理树上问题时，注意区分 `v` 和 `dfn[v]`。
...
$ \mathrm{图不联通的情况数} \to \mathrm{枚举1号节点联通块的所有大小} \times \mathrm{枚举剩余点任意连接的方案} $。[5147]
...
树上背包时如果用 $ \mathrm{size} $ 限制循环的范围__（注意：应遍历父节点和子节点的值来主动更新父节点的值）__，此时虽然是3重循环，但是时间复杂度为 $ O(n^2) $
...
这是预处理区间 $ \mathrm{min}/\mathrm{max} $ 的代码：
...
当一个问题的答案是关于一些区间的状态（值）的函数时（有时出现区间重叠也是可以的），且区间的贡献是关于位置和定值数组的的函数时，考虑用一个类似于前缀和的函数 $ f(\mathrm{pos}) \to \mathrm{value} $ 表示区间，如果有多（类）区间相互影响，考虑构造运算 $ f \oplus g $ 表示整体的局面。[3665]
...
网络流中一系列的边可以认为是对其容量求最小值（最小割），此时可以用流量为 $ \infin $ 的边表示不等式约束关系（不可割）。[5279]